      ...
      @if (loading()) {
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-3 bg-slate-900/40 backdrop-blur-sm px-4 py-2 rounded-2xl border border-slate-700/50 shadow-lg">
            <span class="text-sm font-bold text-slate-200">Score: {{ equityScore() }}%</span>
          </div>
        </div>
          ...
          <div class="col-span-12 lg:col-span-4 space-y-8">
            <div class="bg-slate-900/40 backdrop-blur-sm p-8 rounded-2xl border border-slate-700/50 shadow-lg text-center">
              ...
              <div class="flex justify-center mb-4">
                <app-equity-widget [score]="equityScore()" />
              </div>
              <div class="text-2xl font-bold text-slate-100">{{ equityScore() }}%</div>
...
export class EmployeeEquityComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.employeeService.getMeEquity().subscribe({
      next: (res) => {
        this.equityScore.set(typeof res?.score === 'number' ? res.score : 85);
        this.loading.set(false);
      },
      error: () => {
        this.employeeService.getMe().subscribe({
          next: (p) => { this.equityScore.set(Number(p?.equityScore ?? 85)); this.loading.set(false); },
          error: () => { this.equityScore.set(85); this.loading.set(false); },
        });
      },
    });
  }
