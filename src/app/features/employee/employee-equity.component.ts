  ...
  template: `
    <div class="space-y-8">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-slate-900">My Equity Analysis</h1>
          <p class="text-slate-500">Understand how your schedule is balanced compared to the team.</p>
        </div>
        <div class="flex items-center gap-3 bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm">
          <span class="text-sm font-bold text-slate-700">Rank: 3rd / 24</span>
        </div>
      </div>
      ...
      <div class="grid grid-cols-12 gap-8">
        <div class="col-span-12 lg:col-span-4 space-y-8">
          <div class="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm text-center">
            ...
            <div class="text-4xl font-bold text-slate-900">{{ employee.equityScore }}%</div>
            <p class="mt-6 text-xs text-slate-500">Your score is calculated based on shift rotation, weekend fairness, and break compliance.</p>
          </div>
        </div>

        <div class="col-span-12 lg:col-span-8 space-y-8">
          <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            <h3 class="font-bold text-slate-900 mb-8">Score Components</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              @for (comp of EQUITY_COMPONENTS; track comp.name) {
                <div class="space-y-4">
                  <div class="flex items-center justify-between">
                    <span class="text-sm font-bold text-slate-700">{{ comp.name }}</span>
                    <span class="text-sm font-bold text-slate-900">{{ comp.score }}%</span>
                  </div>
